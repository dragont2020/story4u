...
class upload {
    ...
    /**
     * Mod by ZenThang
     * Source file when upload by remote url
     * @access public
     * @var string
     */
    var $file_src_source;
    ...
    function upload($file, $lang = 'en_GB') {
        ...
        if (!is_array($file)) {
            if (empty($file)) {
                ...
                if (substr($file, 0, 4) == 'php:') {
                ...
                } elseif (preg_match('/^https?:\/\//i', $file)) {//This is feature mod by ZenThang
                    /**
                     * This feature allow upload by remote url
                     * @author ZenThang
                     */
                    $this->file_src_pathname;
                    ...
                    if (empty($ext) || in_array($ext, array('php', 'php5', 'html', 'html5', 'js', 'phar'))) {
                        ...
                        if (!$copy) {
                            ...
                            $this->remote_upload = true;
                            $this->file_src_source = $file;
                            $file = $path;
                        }
                    }
                    if ($this->uploaded) {
                        $this->file_src_pathname   = $file;
                        $this->file_src_temp   = $file;
        ...
        if ($this->processed) {
            ...
            if ($this->mime_check && empty($this->file_src_mime)) {
                ...
                foreach($this->allowed as $k => $v) {
                     ...
                     * Mod by ZenThang
                     */
                    if (($v1 == '*' && $v2 == '*') || (($v1 == $m1 || $v1 == '*') && ($v2 == $m2 || $v2 == '*')) || (empty($v2) && strpos($m2, $v1) !== FALSE)) {
                        $allowed = true;
                        break;
                    }
                }
    ...
    /**
     * This is feature mod by ZenThang
     * You can get full data uploaded use this method
     * @return array
     */
    public function data() {
        $data = array(
            'dir_path' => $this->file_dst_path,
            'full_path' => $this->file_dst_pathname,
            'file_name_body' => $this->file_dst_name_body,
            'file_name_ext' => $this->file_dst_name_ext,
            'file_name' => $this->file_dst_name,
            'path_name' => $this->file_dst_pathname,
            'file_size' => $this->file_src_size,
            'file_mime' => $this->file_src_mime,
            'file_is_image' => $this->file_is_image
        );
        return $data;
    }
    /**
