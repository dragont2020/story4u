...
$functionDataTable = function ($list, $pos) {
    ...
    foreach ($list as $item) {
        ...
        $navBar .= '</ul></div>';
        if($item['num_post']!=0)$num = '('.$item['num_post'].')'; else $num = '';
        echo('<td>' . $navBar . '</td>
        <td>' . (!empty($item['name']) ? '<a href="' . $item['full_url'] . '" title="' . $item['title'] . '">' . $item['name'] . '</a>' : '<a href="' . $item['full_url'] . '" title="' . $item['title'] . '"><i>(Chưa có tiêu đề)</i></a>') . $num. '</td>
        <td>' . (!empty($item['cat']) ? '<a href="' . $item['cat']['full_url'] . '" title="' . $item['cat']['title'] . '">' . $item['cat']['name'] . '</a>' : '<i>Trang chủ</i>') . '</td>
        <td>' . (!empty($item['user']) ? '<a href="' . HOME . '/admin/members/editor?id=' . $item['uid'] . '">' . $item['user']['nickname'] . '</a>' : '<i>N/A</i>') . '</td>
...
ZenView::section(ZenView::get_title(true), function () use ($functionDataTable) {
    ...
    ' . $filter_cat_menu;
    ZenView::block('Posts in <a href="' . ZenView::$D['blog']['full_url'] . '" target="_blank">' . ZenView::$D['blog']['name'] . '</a> <i>(' . $block_title_post . ')</i>', function () use ($functionDataTable) {
    ...
    ZenView::block('Categories in <a href="' . ZenView::$D['blog']['full_url'] . '" target="_blank">' . ZenView::$D['blog']['name'] . '</a> <i>(' . $block_title_cat . ')</i>', function () use ($functionDataTable) {
