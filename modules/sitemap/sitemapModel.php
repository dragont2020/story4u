...
Class sitemapModel Extends ZenModel
{
    ...
    function get_last_update() {
        $sql = "SELECT * FROM " . tb() . "blogs where `type` = 'post' and `status`='0' order by `time` ASC limit 1";
    ...
    public function get_folders() {
        $sql = "SELECT * FROM " . tb() . "blogs where `type` = 'folder' and `status`='0' order by `time` ASC";
        $query = $this->db->query($sql);
        if (!$this->db->num_row($query)) {
            return array();
        }
